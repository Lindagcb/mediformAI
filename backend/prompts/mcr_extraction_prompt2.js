...
11. When extracting DATES:
    - Detect dates even if written outside boxes, beside labels, above/below table rows, or diagonally across the page.
    - Accept ANY handwritten date format, including: DD/MM/YYYY, DD/MM/YY, D/M/YY, DD-MM-YY, DD.MM.YY, “6/11/25”, “6/11”, “6 11 25”, or similar.
    - When the year is missing (e.g., “6/11”), infer NOTHING — return the exact handwriting.
    - If a date appears near (but not inside) a field’s region, ASSOCIATE IT with that field.
    - If handwriting spans multiple table columns, ALWAYS extract the date(s), even if misaligned.
    - NEVER skip dates just because they are not inside the printed box.
12. NEVER reformat or “rewrite” any handwriting, including abbreviations like “NIL”, “APH”, “PPH”, “Hx”, “BP”, “C/S”. Return exactly what is written.


SPECIAL BOOLEAN RULES FOR CHECKBOX SECTIONS:
- For the following checkbox groups ALWAYS return true or false, NEVER strings:
    • Singleton
    • Multiple Pregnancy
    ...
    • EDD Method: SF
    • EDD Method: LNMP
- A tick (✓), cross (✗), filled box, "Yes", or "Pos" → true
- Blank, "No", "Neg", unclear/ambiguous → false

FOR THE OBSTETRIC AND NEONATAL HISTORY TABLE:
- Extract EVERY row, even if handwriting overlaps or does not align to grid lines.
- Extract miscarriage, ectopic pregnancy, stillbirth, or any other outcome as a valid row.
- If a row is written partially outside the table borders, STILL extract it as a separate row.
- Do NOT skip rows that do not have weight, sex, or outcome—return them as empty strings.
- NEVER infer missing data; keep missing fields "".
- If dates (e.g., 6/11/25, 18/06/25, 25/08/25) appear beside the topic, above the line, or handwritten in a cluster, extract ALL of them in the order of appearance.
- Do NOT ignore dates written diagonally, squeezed into margins, or repeated across multiple lines.
